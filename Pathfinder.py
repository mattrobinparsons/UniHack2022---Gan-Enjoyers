...
class Player():
    ...
    def checkCollisions(self):
        ...
        for ep in endPoints:
            if self.collectedCount == NUM_CHECKPOINTS and self.rect.colliderect(ep.rect):
                self.score+=TICK_MAX/2 * NUM_CHECKPOINTS
    ...
    def makeBrain(self, checkpointNum):
        ...
        #hidden layers
        brain.add(Dense(8, input_dim = NUM_CHECKPOINTS*3 + 4, activation='relu',kernel_initializer=initializers.random_uniform(), bias_initializer=initializers.zeros())) 
        # brain.add(Dense(4, activation='relu', kernel_initializer=initializers.random_uniform(), bias_initializer=initializers.zeros())) 
    ...
    def think(self): #endpoint and boxes are global variables 
        ...
        inputs.append((endPoints[0].x + endPoints[0].width/2)/SCREEN_WIDTH)
        inputs.append((endPoints[0].y + endPoints[0].height/2)/SCREEN_HEIGHT)

        for i, cp in enumerate(checkPoints):
            inputs.append((cp.x + cp.width/2)/SCREEN_WIDTH)
            inputs.append((cp.y + cp.height/2)/SCREEN_HEIGHT)
...
def generatePlayers(brain):
    ...
    for i in range(NUM_PLAYERS):
        ...
        if brain:
            players.append(Player(playerX, playerY, playerWidth, playerHeight, mutate(brain, 0.1)))
        else:
